...
| On-S3 layout | (v2, post phase 7) Manifests per-image at `s3://<bucket>/oci/<safe-ref>/<digest>/` (oci-layout, index.json); blobs deduped to a content-addressed bucket-level prefix `s3://<bucket>/blobs/sha256/<hex>`. Two images with a shared layer write to the same blob key, so the second push HEADs and skips. | Bandwidth-saving on overlapping pushes (parallel HEADs against the shared prefix). Trade: cleanup needs reference counting, deferred to a future `images gc` (orphaned blobs accumulate over time but are otherwise harmless). |
